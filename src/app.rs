		...
		<div style="display:flex;justify-content:center">
			<div>
				<ActionForm action=create_election>
					...
					<For
						each=move || {
							candidates.get().into_iter().enumerate().collect::<Vec<_>>()
						}
						key=move |(ind, _)| *ind
							...
							view! {
								<Show when=move || show_candidate.get()>
									<div style="margin:5px">
										<label for=move || {
											format!("candidate_{}", ind)
										}>{move || format!("Candidate {}:", ind + 1)}</label>
										<input
											...
											on:input:target=move |event| {
												candidate.set(event.target().value());
												if (event.target().value().len() > 0)
													&& (ind == (candidates.read_untracked().len() - 1))
												{
													candidates
														.write()
														.push((RwSignal::new(true), RwSignal::new(String::new())))
					...
					<input
						type="submit"
						on:click:target=move |event| {
							let _ = event
								.target()
								.form()
								.expect("form to be connected")
								.request_submit();
							event.target().set_disabled(true);
							event.target().set_value("Submitting…");
						}
					/>
				</ActionForm>
				<ExtraInfo />
...
fn AdminPage() -> impl IntoView {
	...
	view! {
		<div style="display:flex;justify-content:center">
			<div>
				<Suspense fallback=move || view! { <p>"Fetching election details..."</p> }>
					<h1>
						"Admin page for '"
						{Suspend::new(async move { election_info.await.unwrap().election_name })}
						"'"
					</h1>
					<div style="display: inline-block;">
						<p>
							{move || Suspend::new(async move {
								view! {
									<A href=election_info.await.unwrap().admin_path>"Admin link"</A>
								}
							})}
						</p>
						<p>
							{move || Suspend::new(async move {
								view! {
									<A href=election_info.await.unwrap().vote_path>"Vote link"</A>
								}
							})}
						</p>
						<p>
							{move || Suspend::new(async move {
								view! {
									<A href=election_info
											.await
											.unwrap()
											.result_path>"Results link"</A>
								}
							})}
						</p>
					</div>
...
fn VoteThanksPage() -> impl IntoView {
	view! {
		...
		<div style="text-align: center">
			<p>
				"If the vote administrator has shared the results url with you, you can view the results there."
			</p>
		</div>
	}
}
...
fn ResultPage() -> impl IntoView {
	...
	view! {
		<Suspense fallback=move || {
			view! { <p>"Fetching election results..."</p> }
		}>
					...
					<div style="text-align: center">
						...
						<table class="defeat-matrix">
							...
							<tbody>
								{results
									...
									.iter()
									.enumerate()
									.map(|(row_ind, defeats_row)| {
										view! {
											<tr>
												<th scope="row">{results.candidates[row_ind].clone()}</th>
												{defeats_row
													.iter()
													.map(|defeat| view! { <td>{*defeat}</td> })
													.collect_view()}
											</tr>
										}
									})
						...
						{move || {
							if ranked_candidates[0].len() > 1 {
								view! {
									<h4>
										"No winner, tied between: "
										{ranked_candidates[0].join(", ")}
									</h4>
								}
							} else {
								view! { <h4>"Winner: " {ranked_candidates[0][0].clone()}</h4> }
							}
						}}
					</div>
				}
		...
		<div class="extra-info-container">
			<details>
				...
				<div>
					...
					<p>
						"It uses "
						<a href="https://en.wikipedia.org/wiki/Ranked_pairs">
							"Tideman's Ranked Pairs method"
						</a>
						" (a Condorcet voting system) to compute a global ranking from the individual rankings of the votes."
					</p>
				</div>
			</details>
		</div>
